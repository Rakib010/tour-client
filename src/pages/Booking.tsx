...
export default function Booking() {
  ...
  const { id } = useParams();
  const { data, isLoading, isError } = useGetTourQuery(
    { _id: id },
    { skip: !id }
  );
  ...
  const totalAmount = useMemo(() => {
    const price = Number(tourData?.costFrom) || 0;
    return guestCount * price;
  }, [guestCount, tourData?.costFrom]);

  const shortDescription = useMemo(() => {
    const text = String(tourData?.description || "");
  ...
  const handleBooking = async () => {
    ...
    try {
      ...
      toast.error(
        res?.message ||
          "No payment URL returned. Check SSLCommerz credentials on the server."
      );
    } catch (error) {
                ...
                <div>
                  ...
                  <div className="flex items-center space-x-4">
                    <button
                      ...
                      disabled={guestCount <= 1 || isBooking}
                    ...
                    <button
                      ...
                      disabled={hasReachedGuestLimit || isBooking}
                      className="w-10 h-10 rounded-full border border-gray-300 flex items-center justify-center hover:bg-gray-50 transition-colors disabled:opacity-50"
                    >
                      +
                    </button>
